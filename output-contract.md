...
Before writing a recognized wrong question into durable files, ask the learner to confirm:

1. Which knowledge points should be attached to the wrong-question knowledge page. The learner may write the knowledge points directly instead of choosing from candidates.
2. The cause tag, choosing exactly one displayed option:
   - `1 知识点不会`
   - `2 知识点会但是做题思路不对`

At the confirmation gate, do not display the question stem/options back to the learner. Preserve them exactly in structured data for storage, but show only:

- exact explanations, if any, for visibility only;
...
Do not run `scripts/ingest-mistakes.js` until `user_confirmed` is true.
